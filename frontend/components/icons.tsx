// Ícones extraídos 1:1 do Dr. Alecrim.dc.html (mesmos paths SVG da landing original).
...
export function Check({ className, size = 17 }: IconProps) {
  return (
...
const SERVICE_PATHS: Record<string, ReactNode> = {
  ...
  "acao-trabalhista": <path d="M9 6l6 6-6 6M15 6l6 6-6 6" />,
  "acao-civel": (
...
export function ServiceIcon({ slug, className, size = 28 }: IconProps & { slug: string }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round" className={className}>
      {SERVICE_PATHS[slug] ?? FALLBACK_PATH}
    </svg>
  );
}
